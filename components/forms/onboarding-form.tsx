...
interface OnboardingFormProps {
    initialPlanType?: PlanType | null;
}

export function OnboardingForm({ initialPlanType = null }: OnboardingFormProps) {
    ...
    // Plan type state
    const [planType, setPlanType] = useState<PlanType | null>(initialPlanType);
    const [planLimits, setPlanLimits] = useState<ReturnType<typeof getPlanLimits> | null>(
        initialPlanType ? getPlanLimits(initialPlanType) : null
    );
    ...
    // Fetch user's plan type if not provided
    useEffect(() => {
        if (!initialPlanType && isLoaded && user?.id) {

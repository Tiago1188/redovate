...
-- ===================================
    ...
    CREATE TABLE IF NOT EXISTS business_industries(
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        name TEXT NOT NULL UNIQUE, --e.g.Construction
    slug TEXT NOT NULL UNIQUE, --e.g.construction
    description TEXT,
        created_at TIMESTAMP DEFAULT now(),
        updated_at TIMESTAMP DEFAULT now()
    );
...
CREATE TABLE IF NOT EXISTS businesses(
    ...
    dns_verification_token TEXT, --TXT verification token
    verified BOOLEAN DEFAULT false, --domain verified ?
        verified_date TIMESTAMP,
        verified_method TEXT, --TXT, CNAME, etc.
...
--Updated fulltext search uses industry + business fields
...
DROP TRIGGER IF EXISTS business_industries_updated_at ON business_industries;
...
DROP TRIGGER IF EXISTS business_templates_updated_at ON business_templates;
CREATE TRIGGER business_templates_updated_at
BEFORE UPDATE ON business_templates
FOR EACH ROW EXECUTE FUNCTION update_timestamp();

DROP TRIGGER IF EXISTS template_sections_updated_at ON template_sections;
CREATE TRIGGER template_sections_updated_at
BEFORE UPDATE ON template_sections
FOR EACH ROW EXECUTE FUNCTION update_timestamp();

--business_section_status intentionally does NOT use updated_at trigger
